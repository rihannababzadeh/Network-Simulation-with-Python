...
The loss is created based on the packet ID
"""
...
def loss_dist(packet_id):
    ...
    """
    # Define bursty loss behavior
    burst_start = 2  # Start of burst (packet ID)
    burst_end = 20    # End of burst (packet ID)
    burst_loss_rate = 0.9  # Loss rate during burst
    normal_loss_rate = 0.1  # Normal loss rate outside the burst (10%)

    # Check if the packet falls within the burst range
    if burst_start <= packet_id <= burst_end:
        return burst_loss_rate
    else:
        return normal_loss_rate
...
port1 = Port(env, rate)
print("{} port object created.", port1)
...
# Create a new function with the packet_id argument bound with partial
# the loss rate will be calculated based on the number of packets sent by the pg1 packet generator instance
wire1 = Wire(env, partial(random.gauss, 0.1, 0.02), loss_dist = partial(loss_dist, packet_id=pg1.packets_sent), wire_id=1, debug=True)
